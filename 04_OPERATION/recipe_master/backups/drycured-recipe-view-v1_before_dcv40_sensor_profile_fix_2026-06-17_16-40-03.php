...
function dcv5_get_recipe_profile($post_id, $code = '') {
    ...
    if (function_exists('dcv40_existing_display_data_adapter_profile')) {
        $profile = dcv40_existing_display_data_adapter_profile($profile, $code, $post_id);
    }
...
/**
 * DCV40 — Existing display data adapter preview
 *
 * Ne renderira novi HTML.
 * Ne mijenja dizajn.
 * Ne mijenja redoslijed blokova.
 * Samo puni postojeći dcv5/dcv6 profil točnijim podacima.
 *
 * Aktivno samo za pilot 2976 kada je URL parametar:
 * ?dc_data_adapter_preview=2976
 */
if (!function_exists('dcv40_existing_display_data_adapter_preview_enabled')) {
    function dcv40_existing_display_data_adapter_preview_enabled($post_id, $code) {
        if ((int) $post_id !== 2976 && $code !== 'HR-SL-005') {
            return false;
        }
...
if (!function_exists('dcv40_fmt_kg')) {
    function dcv40_fmt_kg($kg) {
        $n = is_numeric($kg) ? (float) $kg : 0.0;
        return number_format($n, 2, ',', '.') . ' kg';
    }
}

if (!function_exists('dcv40_fmt_percent')) {
    function dcv40_fmt_percent($amount, $base = 10.0) {
        $n = is_numeric($amount) ? (float) $amount : 0.0;
        if ($base <= 0) {
            $base = 10.0;
        }
        return rtrim(rtrim(number_format(($n / $base) * 100, 2, ',', '.'), '0'), ',') . ' %';
    }
}

if (!function_exists('dcv40_existing_display_data_adapter_profile')) {
    function dcv40_existing_display_data_adapter_profile($profile, $code, $post_id = 0) {
        if (!is_array($profile) || !dcv40_existing_display_data_adapter_preview_enabled($post_id, $code)) {
            return $profile;
        }

        $vp_raw = get_post_meta($post_id, '_dry_verified_process', true);
        $vp = is_string($vp_raw) ? json_decode($vp_raw, true) : null;
        if (!is_array($vp)) {
            return $profile;
        }

        $base = 10.0;
        $profile['code'] = 'HR-SL-005';
        $profile['title'] = 'Slavonska domaća kobasica';
        $profile['region'] = $vp['regija'] ?? ($profile['region'] ?? 'Slavonija');
        $profile['type'] = $profile['type'] ?? 'Kobasica';

        $materials = [];
        if (!empty($vp['raw_materials']['krto_meso_detalji']) && is_array($vp['raw_materials']['krto_meso_detalji'])) {
            foreach ($vp['raw_materials']['krto_meso_detalji'] as $m) {
                $kg = $m['kg'] ?? null;
                if ($kg === null || $kg === '') {
                    continue;
                }
                $materials[] = [
                    'name' => $m['dio'] ?? 'Svinjsko meso',
                    'amount' => dcv40_fmt_kg($kg),
                    'percent' => dcv40_fmt_percent($kg, $base),
                    'rate' => '',
                ];
            }
        }

        if (!empty($vp['raw_materials']['slanina_detalji']) && is_array($vp['raw_materials']['slanina_detalji'])) {
            foreach ($vp['raw_materials']['slanina_detalji'] as $m) {
                $kg = $m['kg'] ?? null;
                if ($kg === null || $kg === '') {
                    continue;
                }
                $materials[] = [
                    'name' => $m['dio'] ?? 'Tvrda slanina',
                    'amount' => dcv40_fmt_kg($kg),
                    'percent' => dcv40_fmt_percent($kg, $base),
                    'rate' => '',
                ];
        ...
        $spices = [];
        if (!empty($vp['ingredients']) && is_array($vp['ingredients'])) {
            foreach ($vp['ingredients'] as $ing) {
                $name = trim((string) ($ing['naziv'] ?? ''));
                if ($name === '') {
                    continue;
                }

                $low = mb_strtolower($name, 'UTF-8');
                if (strpos($low, 'češnjak') !== false || strpos($low, 'cesnjak') !== false) {
                    continue;
                }
                ...
                if (isset($ing['kolicina_g'])) {
                    $g = (float) $ing['kolicina_g'];
                    $amount = rtrim(rtrim(number_format($g, 1, ',', '.'), '0'), ',') . ' g';
                    $rate = rtrim(rtrim(number_format($g / $base, 2, ',', '.'), '0'), ',') . ' g/kg';
                    $percent = rtrim(rtrim(number_format(($g / 1000 / $base) * 100, 3, ',', '.'), '0'), ',') . ' %';
                } elseif (isset($ing['kolicina_g_min'], $ing['kolicina_g_max'])) {
                    $min = (float) $ing['kolicina_g_min'];
                    $max = (float) $ing['kolicina_g_max'];
                    $amount = rtrim(rtrim(number_format($min, 1, ',', '.'), '0'), ',') . '–' . rtrim(rtrim(number_format($max, 1, ',', '.'), '0'), ',') . ' g';
                    $rate = rtrim(rtrim(number_format($min / $base, 2, ',', '.'), '0'), ',') . '–' . rtrim(rtrim(number_format($max / $base, 2, ',', '.'), '0'), ',') . ' g/kg';
        ...
        $profile['liquids'] = [
            [
                'name' => 'Češnjak za procijeđenu aromatsku tekućinu',
                'amount' => '30–50 g',
                'percent' => '0,3–0,5 %',
                'rate' => '3–5 g/kg',
                'note' => 'Češnjak zgnječiti, preliti prokuhanom i ohlađenom vodom, namakati 20–30 minuta, promiješati i procijediti. U nadjev dodati samo procijeđenu aromatsku tekućinu.',
            ],
        ];

        $meat_plate = $vp['mljevenje']['krto_meso_resetka_mm'] ?? '';
        $fat_cut = $vp['mljevenje']['slanina_rezanje'] ?? '';
        $fat_condition = $vp['mljevenje']['slanina_preduvjet'] ?? '';

        $casing_type = $vp['casing']['type'] ?? 'svinjska tanka crijeva';
        $casing_diameter = $vp['casing']['diameter_mm'] ?? '';
        ...
        $profile['timeline'] = [
            [
                'day' => 'Dan 0',
                'title' => 'Priprema mesa',
                'text' => 'Sirovinu dobro ohladiti, očistiti od neželjenog veziva i pripremiti za mljevenje ili rezanje.',
                'critical' => 'Meso i slanina moraju ostati hladni tijekom cijele obrade.',
            ],
            [
                'day' => 'Dan 0',
                'title' => 'Mljevenje i obrada slanine',
                'text' => 'Krto meso mljeti na rešetku ' . $meat_plate . ' mm. Tvrdu leđnu slaninu obraditi: ' . $fat_cut . '.',
                'critical' => 'Slanina treba biti ' . $fat_condition . '; ako se masa počne lijepiti, obradu prekinuti i sirovinu vratiti na hlađenje.',
            ],
            [
                'day' => 'Dan 0',
                'title' => 'Miješanje',
                'text' => 'Dodati sol, papriku, papar i procijeđenu aromatsku tekućinu od češnjaka te miješati dok se masa ne poveže i ravnomjerno oboji.',
                'critical' => 'Nadjev mora ostati hladan i ne smije postati pastozan.',
            ],
            [
                'day' => 'Dan 0',
                'title' => 'Punjenje',
                'text' => 'Puniti u ' . $casing_type . ($casing_diameter ? ' kalibra ' . $casing_diameter . ' mm' : '') . ', oblikovati komade duljine ' . $casing_length . ' cm i odmah ukloniti vidljive mjehuriće zraka.',
                'critical' => 'Puniti ravnomjerno i čvrsto, ali bez pucanja crijeva.',
            ],
            [
                'day' => 'Dan 1–3',
                'title' => 'Predsušenje',
                'text' => 'Kobasice objesiti u hladan i prozračan prostor dok površina ne bude suha na dodir.',
                'critical' => 'Ne počinjati dimljenje dok je površina mokra.',
            ],
            [
        ...
        $profile['phase_specs'] = [
            'Mljevenje' => [
                'Trajanje' => 'odmah nakon hlađenja',
                'Temperatura' => '0–4 °C',
                'Rešetka' => $meat_plate . ' mm',
                'Slanina' => $fat_cut,
                'Cilj' => 'jasan presjek bez razmazane masti',
            ],
            'Miješanje' => [
                'Trajanje' => 'dok se masa ne poveže',
                'Temperatura' => '0–6 °C',
                'Kontrola' => 'nadjev ne smije postati topao ni pastozan',
                'Cilj' => 'ravnomjeran raspored soli, paprike, papra i češnjakove arome',
            ],
            'Punjenje' => [
                'Trajanje' => 'isti dan',
                'Temperatura' => '0–6 °C',
                'Crijeva' => trim($casing_type . ($casing_diameter ? ' ' . $casing_diameter . ' mm' : '')),
